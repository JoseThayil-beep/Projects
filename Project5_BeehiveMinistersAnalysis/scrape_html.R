scrape_search_results <- function(page_path) {
  ...
  map_df(nodes, function(node) {
    # Title
    title_element <- node %>% html_element(".views-field.views-field-title")
    title <- title_element %>% html_text2()
    
    # Date and time
    date_element <- node %>% html_element(".views-field views-field-field-issue-date, time")
    ...
    # Ministers
    minister <- node %>%
      html_elements(".views-field.views-field-field-ministers") %>%
      html_text2() %>%
    ...
    # Portfolios
    portfolio <- node %>%
      html_elements(".views-field.views-field-field-portfolios") %>%
      html_text2() %>%
      # trim out excess white space noise, and collapse them using a semicolon
      str_trim() %>%
      paste(collapse = ";")
